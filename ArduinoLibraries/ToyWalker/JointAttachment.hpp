...
{
public:
	JointAttachment(Joint & joint, JointedSegment & A, Vector positionA, Vector axisA, JointedSegment & B, MatrixAffine transformAtoB, double radiansAtTransform)
	...
	MatrixAffine AToB(double radians)
	{
		return _AToB
	...
	MatrixAffine BToA(double radians) { return AToB(radians).inverse(); }
	MatrixAffine toOther(JointedSegment * self, double radians)
	{
